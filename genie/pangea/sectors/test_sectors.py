...
def test_sector_api_systemerror(pangea_url):
    api = f"{pangea_url}/sectors"
...
def test_sector_api_authorizationerror(pangea_url):
    api = f"{pangea_url}/sectors"
    # Create a mock object that raises an exception
    mock_response = Mock()
    ...
    mock_get.assert_called_once_with(f"{pangea_url}/sectors")    
...
def test_subsectors_api(sectorname, subsectorcount,pangea_url):
    api = f"{pangea_url}/sectors/subsectors?sector="+sectorname
    response = requests.get(api)
...
def test_subsectors_api_invalidsector(pangea_url):
    api = f"{pangea_url}/sectors/subsectors?sector=ABC"
    response = requests.get(api)
    # Displays the paprticular sector's subsectors (like Health Care subsectors, Materials subsectors and so on...)
    assert response.status_code == 200
    json_data = response.json()
...
def test_subsector_api_systemerror(pangea_url):
    api = f"{pangea_url}/sectors/subsectors?sector=Utilities"
    # Create a mock object that raises an exception
    mock_response = Mock()
    mock_response.status_code = 500  # Simulate a failed response
...
def test_subsector_api_authorizationerror(pangea_url):
    api = f"{pangea_url}/sectors/subsectors?sector=Utilities"
    # Create a mock object that raises an exception
    mock_response = Mock()
    mock_response.status_code = 401 # Simulate a failed response
...
def test_marketcaps_api(pangea_url):
    # calling marketcaps api
    api = f"{pangea_url}/sectors/marketcap"
    response = requests.get(api) 
...
def test_marketcap_api_authorizationerror(pangea_url):
    api = f"{pangea_url}/sectors/marketcap"
    # Create a mock object that raises an exception
    mock_response = Mock()
    mock_response.status_code = 401  # Simulate a failed response
...
def test_sectormarketcap_api(sectorname,marketcapital,pangea_url):
    api=f"{pangea_url}/sectors/"+sectorname+"/marketcap"
    response = requests.get(api)
    assert response.status_code == 200
    json_data = response.json()
...
@pytest.mark.skip 
def test_sectormarketcap_api_invalidsector(pangea_url):
    api=f"{pangea_url}/sectors/ABC/marketcap"
    response = requests.get(api)
    assert response.status_code == 200
    json_data = response.json()
    list_size = len(json_data)
...
def test_sectormarketcap_api_systemerror(pangea_url):
    api=f"{pangea_url}/sectors/Utilities/marketcap"
    # Create a mock object that raises an exception
    mock_response = Mock()
    mock_response.status_code = 500  # Simulate a failed response
...
def test_sectormarketcap_api_authorizationerror(pangea_url):
    api=f"{pangea_url}/sectors/Utilities/marketcap"
    # Create a mock object that raises an exception
    mock_response = Mock()
    mock_response.status_code = 401  # Simulate a failed response
